...
export default function DoctorList() {
    ...
    const showModal = (row) => {
        delete row.id;
        row.Active=    <Switch defaultChecked />;
        row.Verify=   row.Is_Verified ? "Verified" : <Switch defaultChecked />;
        setSelectedRow(row);
        setOpen(true);
        console.log(row);
    }
    const columns = [
        ...
        {
            field: 'Is_Verified', flex: 1, headerName: 'Verify Actions', 
        },
        {
            field: 'Is_Active', flex: 1, headerName: 'Active Actions'
        },
        {
